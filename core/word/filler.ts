...
/** Path of the tag-ified factory template (generated by scripts/prepare-word-template.cjs) */
const FILLABLE_TEMPLATE_PATH = resolveResourcePath('resources', 'templates', 'deviation-report-fillable.docx');
 ...
 */
export function renderTemplate(data: DocxFillData, templatePath: string = FILLABLE_TEMPLATE_PATH): Buffer {
 ...
 */
export function exportDocxToFile(report: DeviationReport, outputPath: string): string {
  const data = buildDocxData(report);
  const buffer = renderTemplate(data);
  fs.writeFileSync(outputPath, buffer);
  log.info('Word report exported', { outputPath, size: buffer.length });
  return outputPath;
}
